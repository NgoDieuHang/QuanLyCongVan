...
$(document).ready(function () {
    try {
        InitalizeThingsAchieved();
        InitEventThingsAchieved();
    }
    catch (e) {
 ...
 * Output       :   
 */
function InitalizeThingsAchieved() {
    try {
        InitPage();
        ...
        jMessage(0, function (ok) {
        }, '<b>InitalizeThingsAchieved:</b> ' + e.message, 4);
    }
}
 ...
 * Output       :   
 */
function InitEventThingsAchieved() {
    try {
        ...
        jMessage(0, function (ok) {
        }, '<b>InitEventThingsAchieved:</b> ' + e.message, 4);
    }
}
...
/*
 * Cập nhật trạng thái hiên thị của những điều đạt được lên server
 * Author       :   HaLTH - 26/07/2018 - create
 * Param        :   
 * Output       :   
 */
function UpdateShowThingsAchieved(element) {
    try {
        $.ajax({
            type: 'POST',
        ...
        jMessage(0, function (ok) {
        }, '<b>UpdateShowThingsAchieved:</b> ' + e.message, 4);
    }
}

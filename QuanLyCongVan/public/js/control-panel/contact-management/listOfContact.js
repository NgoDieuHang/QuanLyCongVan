...
function InitalizeContacts() {
    try {
        ...
        jMessage(0, function (ok) {
        }, '<b>InitalizeContacts:</b> ' + e.message, 4);
    }
}

/*
 * Khởi tạo các sự kiện của cho trang
 * Author       :   HoangNM - 27/07/2018 - create
 * Param        :   
 * Output       :   
 */
function InitEventContacts() {
    try {
        ...
        $(document).on('change', '.show-contact', function () {
            UpdateShowContact(this);
        });
    }
    catch (e) {
        jMessage(0, function (ok) {
        }, '<b>InitEventContacts:</b> ' + e.message, 4);
    }
}
...
/*
 * Cập nhật trạng thái của Contact lên server
 * Author       :   HoangNM - 23/07/2018 - create
 * Param        :   
 * Output       :   
 */
function UpdateShowContact(element) {
    try {
        $.ajax({
            type: 'POST',
